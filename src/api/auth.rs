...
pub async fn do_login_flow() -> Result<AuthCodePkceSpotify, AppError> {
    ...
    let url_with_code = loop {
        if attempts > 60 { // 2 minutes timeout (assuming 2s intervals)
    ...
    let code = spotify.parse_response_code(&url_with_code)
    ...
    let token = token_guard.clone().ok_or_else(|| AppError::Auth("No token obtained".to_string()))?;
